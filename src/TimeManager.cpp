...
bool TimeManager::isOzoneTimeScheduled() {
    // Если RTC в состоянии ошибки, мы не имеем права запускать озонатор
    if (!rtcValid) return false;

    // Проверяем совпадение дня недели, часов и минут
    if (currentTime.dayOfTheWeek() == scheduleDayOfWeek &&
        ...
        // Если мы уже запускали озонатор СЕГОДНЯ, игнорируем совпадение.
        // Это спасет от ситуации, когда FSM прервал цикл из-за мороза и вернулся
        // в AUTO_CLIMATE в ту же самую минуту (02:00).
